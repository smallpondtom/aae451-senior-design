%% MATLAB AIRCRAFT SIZING CODE (MASC) - GA AIRCRAFT (ELEC)
...
PerformanceInputs.Esb  = 285;       % BATTERY ENERGY DENSITY [W-hr/kg]
...
PropulsionInputs.eta_e      = 0.931;       % ELECTRIC efficiency BATTERY TO SHAFT
%% PAYLOAD PARAMETERS
...
%% SIZE AIRCRAFT
    SizingOutput = SizingIterations_elec(inputs);

%% ECONOMIC MISSION ANALYSIS
   EconMissionOutput = EconMissionFunction_elec(SizingOutput);
...
%    FinalOutput.OpCostOutput = OpCostOutput;
   ReportFunction_elec(FinalOutput);
