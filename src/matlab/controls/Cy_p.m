...
% INPUTS:
    % b_w - wing span (ft)
    % l_v - the horizontal distance from the airplane reference point to the aerodynamic centre of the vertical tail
    % z_v - the vertical distance from the airplane reference point to the aerodynamic centre of the vertical tail
    % alpha - angle of attack

function [Cy_p]=C_p(b_w,l_v,z_v,alpha,two_r_one,eta_v,beta,AR_v,b_v,Z_h,x_over_c_v,lambda_v,S_v,S_w,Lambda_c4_v,Z_w,d,dihedral,wingloc,Z_w1,S_h,S_o)
...
%%The following is used to find the vertical tail component of Cy_beta
...
% From equataion 8.1 on page 8.1
Cy_p=2*((z_v*cos(alpha))-(l_v*sin(alpha)))/b_w*C_y_beta_v;